	...
	msg_S <- paste( "Size of graph title     :", arg_S )
	...
	Label <- paste( dat$ResName, as.character( dat$ResSeq ), sep = "" )
	#print( Label )
	topn <- ifelse( rank( -dat$Cons, ties.method = "min" ) <= arg_t, Label, "" )
	#print( topn )
	#print( order( dat$Cons, decreasing = T ) )
	dat <- transform( dat, Label = topn )
	#print( dat )

	# ggplot
	output <- ggplot( data = dat, aes( x = Number, y = Cons, colour = State, label = Label ) ) +
	#theme_calc() +
	#theme_economist() +
	...
	ylim( 0, 1 ) +
	geom_segment( aes( x = Number, xend = Number, y = 0, yend = Cons ), colour = "gray" ) +
	geom_point( size = arg_s ) +
	geom_label_repel( box.padding = 1, max.overlaps = Inf, show.legend = FALSE ) +
	scale_colour_manual( values = c( "#FF0075", "#2F89FC", "#00BD56" ) )
